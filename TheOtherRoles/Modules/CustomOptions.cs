...
namespace TheOtherRoles
{
    public class CustomOption
    {
        ...
        {
            return translationInfo != null ? translationInfo.GetString() : "";
        }
        ...
        public void updateSelection(int newSelection, bool notifyUsers = true)
        {
            ...
            else if (id == 0 && AmongUsClient.Instance?.AmHost == true && CachedPlayer.LocalPlayer.PlayerControl)
        ...
        public static int pasteFromClipboard()
        {
            ...
            try
            {
                ...
                if (TheOtherRolesPlugin.Version > versionInfo && versionInfo < Version.Parse(TheOtherRolesPlugin.VersionString))
                {
                    ...
                    FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(CachedPlayer.LocalPlayer.PlayerControl, ModTranslation.GetString("GameStart", 9));
                }
                else
                {
            ...
            catch (Exception e)
            {
                ...
                FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(CachedPlayer.LocalPlayer.PlayerControl, string.Format(ModTranslation.GetString("GameStart", 11), errorStr));
    ...
    class LobbyViewSettingsPatch
    {
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType)
        {
            ...
            foreach (var option in relevantOptions)
            {
                ...
                viewSettingsInfoPanel.titleText.text = option.getTitle();
        ...
        public static void createSettingTabs(LobbyViewSettingsPane __instance)
        {
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.Guesser || TORMapOptions.gameMode == CustomGamemodes.Classic)
            {
                ...
                // create TOR settings
                createCustomButton(__instance, next++, "TORSettings", ModTranslation.GetString("Opt-General", 71), CustomOptionType.General);
                // create TOR settings
                createCustomButton(__instance, next++, "RoleOverview", ModTranslation.GetString("Opt-General", 110), (CustomOptionType)99);
                // IMp
                createCustomButton(__instance, next++, "ImpostorSettings", ModTranslation.GetString("Opt-General", 81), CustomOptionType.Impostor);

                // Neutral
                createCustomButton(__instance, next++, "NeutralSettings", ModTranslation.GetString("Opt-General", 80), CustomOptionType.Neutral);
                // Crew
                createCustomButton(__instance, next++, "CrewmateSettings", ModTranslation.GetString("Opt-General", 79), CustomOptionType.Crewmate);
                // Modifier
                createCustomButton(__instance, next++, "ModifierSettings", ModTranslation.GetString("Opt-General", 82), CustomOptionType.Modifier);

            }
            else if (TORMapOptions.gameMode == CustomGamemodes.HideNSeek)
            {
                // create Main HNS settings
                createCustomButton(__instance, next++, "HideNSeekMain", ModTranslation.GetString("Opt-General", 77), CustomOptionType.HideNSeekMain);
                // create HNS Role settings
                createCustomButton(__instance, next++, "HideNSeekRoles", ModTranslation.GetString("Opt-General", 72), CustomOptionType.HideNSeekRoles);
            }
            else if (TORMapOptions.gameMode == CustomGamemodes.PropHunt)
            {
                createCustomButton(__instance, next++, "PropHunt", ModTranslation.GetString("Opt-General", 103), CustomOptionType.PropHunt);
            }
        }
    }
    ...
    class GameOptionsMenuStartPatch
    {
        ...
        private static void createSettingTabs(GameSettingMenu __instance)
        {
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.Guesser || TORMapOptions.gameMode == CustomGamemodes.Classic)
            {
                ...
                // create TOR settings
                createCustomButton(__instance, next++, "TORSettings", ModTranslation.GetString("Opt-General", 71));
                ...
                // IMp
                createCustomButton(__instance, next++, "ImpostorSettings", ModTranslation.GetString("Opt-General", 81));
                createGameOptionsMenu(__instance, CustomOptionType.Impostor, "ImpostorSettings");

                // Neutral
                createCustomButton(__instance, next++, "NeutralSettings", ModTranslation.GetString("Opt-General", 80));
                createGameOptionsMenu(__instance, CustomOptionType.Neutral, "NeutralSettings");
                // Crew
                createCustomButton(__instance, next++, "CrewmateSettings", ModTranslation.GetString("Opt-General", 79));
                createGameOptionsMenu(__instance, CustomOptionType.Crewmate, "CrewmateSettings");
                // Modifier
                createCustomButton(__instance, next++, "ModifierSettings", ModTranslation.GetString("Opt-General", 82));
    ...
    public class AmongUsClientOnPlayerJoinedPatch
    {
        public static void Postfix()
        {
            if (CachedPlayer.LocalPlayer.PlayerControl != null && AmongUsClient.Instance.AmHost)
            {
                GameManager.Instance.LogicOptions.SyncOptions();
                CustomOption.ShareOptionSelections();
            }
    ...
    class GameOptionsDataPatch
    {
        ...
        public static string buildModifierExtras(CustomOption customOption)
        {
            ...
            var quantity = children.Where(o => o.getTitle().Contains("Quantity")).ToList();
        ...
        private static string buildOptionsOfType(CustomOption.CustomOptionType type, bool headerOnly)
        {
            ...
            foreach (var option in options)
            {
                ...
                {
                    string line = $"{option.getTitle()}: {option.selections[option.selection].ToString()}";
                ...
                else if (option.parent.getSelection() > 0)
                {
                    if (option.id == 103) //Deputy
                        ...
                        sb.AppendLine($"- {Helpers.cs(Sidekick.color, ModTranslation.GetRoleName(RoleId.Sidekick).GetString())}: {option.getString()}");
                    else if (option.id == 358) //Prosecutor
                        sb.AppendLine($"- {Helpers.cs(Lawyer.color, ModTranslation.GetRoleName(RoleId.Prosecutor).GetString())}: {option.getString()}");
                }
            }
            ...
            foreach (CustomOption option in options)
            {
                ...
                if (option.parent != null)
                {
                    ...
                    sb.AppendLine(Helpers.cs(c, $"{option.getTitle()}: {option.selections[option.selection].ToString()}"));
                }
                else
                {
                    if (option == CustomOptionHolder.crewmateRolesCountMin)
                    {
                        var optionName = CustomOptionHolder.cs(new Color(204f / 255f, 204f / 255f, 0, 1f), ModTranslation.GetString("Opt-General", 79));
                    ...
                    {
                        var optionName = CustomOptionHolder.cs(new Color(204f / 255f, 204f / 255f, 0, 1f), ModTranslation.GetString("Opt-General", 80));
                    ...
                    {
                        var optionName = CustomOptionHolder.cs(new Color(204f / 255f, 204f / 255f, 0, 1f), ModTranslation.GetString("Opt-General", 81));
                    ...
                    {
                        var optionName = CustomOptionHolder.cs(new Color(204f / 255f, 204f / 255f, 0, 1f), ModTranslation.GetString("Opt-General", 82));
                    ...
                    {
                        sb.AppendLine($"\n{option.getTitle()}: {option.selections[option.selection].ToString()}");
                    }
                }
            }
            return sb.ToString();
        ...
        public static string buildAllOptions(string vanillaSettings = "", bool hideExtras = false)
        {
            ...
            string hudString = counter != 0 && !hideExtras ? Helpers.cs(DateTime.Now.Second % 2 == 0 ? Color.white : Color.red, $"{ModTranslation.GetString("Opt-General", 83)}\n\n") : "";
            ...
            else
            {
                ...
                {
                    case 0:
                        hudString += (!hideExtras ? "" : $"{ModTranslation.GetString("Opt-General", 86)} \n\n") + vanillaSettings;
                        break;
                    case 1:
                        hudString += $"{ModTranslation.GetString("Opt-General", 87)} \n" + buildOptionsOfType(CustomOption.CustomOptionType.General, false);
                        break;
                    case 2:
                        hudString += $"{ModTranslation.GetString("Opt-General", 88)} \n" + buildRoleOptions();
                        break;
                    case 3:
                        hudString += $"{ModTranslation.GetString("Opt-General", 89)} \n" + buildOptionsOfType(CustomOption.CustomOptionType.Impostor, false);
                        break;
                    case 4:
                        hudString += $"{ModTranslation.GetString("Opt-General", 90)} \n" + buildOptionsOfType(CustomOption.CustomOptionType.Neutral, false);
                        break;
                    case 5:
                        hudString += $"{ModTranslation.GetString("Opt-General", 91)} \n" + buildOptionsOfType(CustomOption.CustomOptionType.Crewmate, false);
                        break;
                    case 6:
                        hudString += $"{ModTranslation.GetString("Opt-General", 92)} \n" + buildOptionsOfType(CustomOption.CustomOptionType.Modifier, false);
                        break;
                }
            }

            if (!hideExtras || counter != 0) hudString += $"\n {string.Format(ModTranslation.GetString("Opt-General", 93), counter + 1, maxPage)}";
            return hudString;
        }
    ...
    public class HudManagerUpdate
    {
        ...
        public static void Postfix(HudManager __instance)
        {
            ...
            bool zoomButtonActive = !(CachedPlayer.LocalPlayer.PlayerControl == null || !CachedPlayer.LocalPlayer.PlayerControl.Data.IsDead || (CachedPlayer.LocalPlayer.PlayerControl.Data.Role.IsImpostor && !CustomOptionHolder.deadImpsBlockSabotage.getBool()));
