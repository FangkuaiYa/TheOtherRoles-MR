    ...
    class CreateOptionsPickerPatch {
        ...
        public static bool Prefix(CreateOptionsPicker __instance, ref GameModes mode) {
            ...
            if (gm == CustomGamemodes.Guesser) {
                __instance.GameModeText.text = ModTranslation.GetString("GameModeText", 1);
                TORMapOptions.gameMode = CustomGamemodes.Guesser;
            } else if (gm == CustomGamemodes.HideNSeek) {
                __instance.GameModeText.text = ModTranslation.GetString("GameModeText", 2);
                TORMapOptions.gameMode = CustomGamemodes.HideNSeek;
            } else if (gm == CustomGamemodes.PropHunt) {
                __instance.GameModeText.text = ModTranslation.GetString("GameModeText", 3);
            ...
            for (int i=0; i <= 5; i++) {
                ...
                if (entry != GameModes.None) {
                    ...
                    if (i <= 2)
                        ...
                        chatLanguageButton.Text.text = i == 3 ? ModTranslation.GetString("GameModeText", 1) : ModTranslation.GetString("GameModeText", 2);
                        if (i == 5)
                            chatLanguageButton.Text.text = ModTranslation.GetString("GameModeText", 3);
                    }
